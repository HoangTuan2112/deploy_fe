...
const SliderItems = () => {
	...
	const handleCategorySelect = async (categoryId) => {
		...
		if (categoryId === "") {
			...
			try {
				const response = await axios.get(`https://kltn-0bb604b830af.herokuapp.com/api/v1/quizs/category/${categoryId}`, {
					headers: {
	...
	const fetchAllQuizs = async () => {
		try {
			const response = await axios.get(`https://kltn-0bb604b830af.herokuapp.com/api/v1/quizs/getAllQuizByCategory`, {
				headers: {
		...
		const fetchCategories = async () => {
			try {
				const response = await axios.get(
					'https://kltn-0bb604b830af.herokuapp.com/api/v1/category/getAll',
					{
						headers: {
							'Authorization': `Bearer ${token}`,
							'Content-Type': 'application/json',
