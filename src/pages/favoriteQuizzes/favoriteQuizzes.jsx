...
const FavoriteQuizzes = () => {
    ...
    useEffect(() => {
        const fetchFavoriteQuizzes = async () => {
            try {
                const response = await axios.get('https://kltn-0bb604b830af.herokuapp.com/api/v1/quizs/favorite/user', {
                    headers: {
                ...
                {favoriteQuizzes.map((quiz) => (
                    <Link style={{ color: "#fff" }} to={`/quizdetail/examcontent/${quiz.id}`}>
                        <div className="card" key={quiz.id}>
                            <img
                                src="http://images.unsplash.com/photo-1656618020911-1c7a937175fd?crop=entropy&cs=tinysrgb&fm=jpg&ixid=MnwzMjM4NDZ8MHwxfHJhbmRvbXx8fHx8fHx8fDE2NTc1MzQyNTE&ixlib=rb-1.2.1&q=80"
                                alt="Quiz Banner"
                            />
